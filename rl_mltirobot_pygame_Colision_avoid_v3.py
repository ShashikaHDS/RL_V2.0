...
class RectangleReductionEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            new_position, prev_positions = self._move_robot(self.robot_positions[i], action)
            #print(new_position)
        ...
        # Reward calculation
            ...
            if new_area <= 16:  
                reward += 200000
                self.reward_contribution[0]+=200000
                if self.step_count<=50:
                    reward+=50000
                    #print("step count < 50")
                elif self.step_count<=150:
                    reward+=1000
                    #print(" step count <=150")
                elif self.step_count<=250:
                    reward+=500
                    #print(" step count <=250")
                elif self.step_count<=500:
                    reward-=10
                    #print("step count < 500 ")
                else:
                    reward-=100
                    #print(" >500")
            ...
            for i, position in enumerate(self.robot_positions):
                ...
                if(self.visit_map[position[0],position[1]]>4):
                    ...
                    reward += (int(revisit_penalty*0.001))
                    self.reward_contribution[6]+= (int(revisit_penalty*0.001))
...
# Save the model
model.save("loading/rectangle_reduction_model_PPO_v3_12_29_6_10")
