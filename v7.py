...
class RectangleReductionEnv(gym.Env):
    def __init__(self, grid_map, max_robots, robot_positions, field_of_view, num_robots):
        ...
        self.action_space = gym.spaces.MultiDiscrete([5] * self.max_robots)
        ...
        self.observation_space = gym.spaces.Dict({
            ...
            "robot_positions": gym.spaces.Box(0, max(self.grid_size), shape=(self.max_robots, 2), dtype=np.int32)
        })

        # Known map and rectangle area
        self.known_map = np.zeros_like(self.grid_map)
        self.max_rectangle_area = self.calculate_rectangle_area(self.num_robots)
    ...
    def reset(self):
        ...
        self.num_robots = random.randint(2,6)
        self.grid_map = grid_maps.generate_connected_clusters_map(
        ...
        print([tuple(free_cells[i]) for i in random_indices],[(0,0)*(self.max_robots - self.num_robots)])
        self.robot_positions = np.array([tuple(free_cells[i]) for i in random_indices]+ [(0,0) for i in range(0,self.max_robots - self.num_robots)])
        self.prev_positions = self.robot_positions.copy()
        self.known_map = np.zeros_like(self.grid_map)
        self.max_rectangle_area,free_status,bounds = self.calculate_rectangle_area(self.num_robots)
        self.cumulative_reward = 0
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions[:self.num_robots]):                                                                 
        ...
        new_area,free_status,square_bounds = self.calculate_rectangle_area(self.num_robots)
    ...
    def calculate_rectangle_area(self,no_robots):
        x_coords = self.robot_positions[:no_robots][:, 0]
        y_coords = self.robot_positions[:no_robots][:, 1]
...
env = RectangleReductionEnv(grid_map, max_robots, robot_positions=robot_positions, field_of_view=1, num_robots=num_robots)
...
while True:
    ...
    model.save("loading/rectangle_reduction_model_PPO_2025_01_06_v1")
