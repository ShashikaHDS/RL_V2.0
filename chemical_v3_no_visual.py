...
class ChemicalClean(gym.Env):
    def __init__(self, grid_map, num_robots, robot_positions, field_of_view, chem_fov, rewards=None):
        ...
        self._map_rows = 20
        self._map_cols = 20
        self._map_cluster_range = (30, 60)
        self._map_subcluster_config = {
            "num_range": (4, 8),
            "size_range": (5, 15),
    ...
    def reset(self):
        ...
        self.grid_map = np.array(self._map_gen.generate_main_with_subclusters_map(
            self._map_rows, self._map_cols,
            self._map_cluster_range, self._map_subcluster_config
        ))
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            if self.grid_map[new_position[0]][new_position[1]] == 2:            #obstacles
                reward = self.rewards["obstacle_penalty"]
            ...
            if self.grid_map[new_position[0], new_position[1]] == 1:  # If it's a spill
                ...
                if self.spill_map[new_position[0], new_position[1]] == 0 and self.pre_cleaned==True:
                    reward=self.rewards["consecutive_bonus"]
                    ...
                    reward = self.rewards["clean_spill"]
                    self.pre_cleaned=True
                    ...
                    reward=self.rewards["revisit_penalty"]
                    self.non_spill_exp_count+=1
                    self.pre_cleaned=False


            else:
                reward = self.rewards["explore_penalty"]  # Penalty for moving to non-spill cells
                self.non_spill_exp_count+=1
                self.pre_cleaned=False
        ...
        info = {}
        if done:
            total_spills = int(np.sum(self.grid_map == 1))
            info["cleaned_pct"] = self.cleaned_cell_count / total_spills if total_spills > 0 else 0
            info["cleaned_count"] = self.cleaned_cell_count
            info["total_spills"] = total_spills
...
if __name__ == "__main__":
    ...
    grid_map = np.array(grid_map_call)
    ...
    models_dir = f"models/Chemical_spils_11_06_2025/{int(time.time())}/"
    logdir = f"logs/Chemical_spils_11_06_2025/{int(time.time())}/"
