...
class RectangleReductionEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):                                                                 
            new_position, _ = self._move_robot(self.robot_positions[i], action)
            ...
            if valid_move:
                self.steps_taken[i] += 1                                          #Increment only if the move is valid
        ...
        if self.step_count <= 300000:
            '''
            #check new center inside the new rectangle
            if new_min_x<center_new[0]<new_max_x and new_min_y<center_new[1]<new_max_y:
                reward+=1000
            ...
            #go to center direction until rectangle reach some 
                #???????????????????????????????????
            
            if (new_min_x >= prev_min_x and new_max_x <= prev_max_x and       #rect in rect
        new_min_y >= prev_min_y and new_max_y <= prev_max_y):
                reward += 100
            else:
                reward-=10

            if new_area <= 16:                                                #converge
                reward += 250
                print("---------------------Goal-------------------------")
                ...
                reward += (2 + 2 * (self.max_rectangle_area - new_area))
            elif new_area > self.max_rectangle_area:
                reward -= (20 + 5 * abs(self.max_rectangle_area - new_area))
            
            if new_area>=16:
                for i, position in enumerate(self.robot_positions):           #revisit
                    self.visit_map[position[0], position[1]] += 1
                    if self.visit_map[position[0], position[1]] > 4:
                        reward += -0.1 * self.visit_map[position[0], position[1]]
                        #print(f" Revist panelty: {self.visit_map[position[0],position[1]]}")
            else:
                pass
...
num_robots = 4
field_of_view = 1
grid_map_call = grid_maps.generate_connected_clusters_map(rows=20, cols=20, num_clusters=5, cluster_size_range=(2, 5), min_distance=3)
...
model.save("loading/rectangle_reduction_model_PPO_2025_01_07_v1")
